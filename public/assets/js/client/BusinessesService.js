class BusinessesService {
    ...
    async store(data) {
        try {
            console.log("Preparing to send request to /businesses/store with data:", Array.from(data.entries()));
            const response = await this.requestClient.post('/businesses/store', data, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });
            console.log("Received response from /businesses/store:", response);
            await Swal.fire('Congratulations!', response.message, 'success');
            this.handleRedirect(response.data.redirect_url);
            return response;
        } catch (error) {
            console.error("Error in BusinessesService.store:", {
