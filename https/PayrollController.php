...
public function emailP9(Request $request, $id)
{
    return $this->handleTransaction(function () use ($id, $request) {
        ...
        foreach ($payroll->employeePayrolls as $employeePayroll) {
            ...
            // Prefill all 12 months with 0.00
            $monthlyData = [];
            for ($i = 1; $i <= 12; $i++) {
                $monthlyData[$i] = [
                    'basic_salary' => 0, 'benefits_non_cash' => 0, 'value_of_quarters' => 0,
                    'total_gross_pay' => 0, 'retirement_e1' => 0, 'retirement_e2' => 0,
                    'retirement_e3' => 20000, 'owner_occupied_interest' => 0,
                    'retirement_contribution' => 0, 'chargeable_pay' => 0,
                    'tax_charged' => 0, 'personal_relief' => 2400, 'insurance_relief' => 0,
                    'paye' => 0, 'housing_levy' => 0, 'shif' => 0, 'prmf' => 0,
                    'total_deductions' => 0,
                ];
            }

            $totals = array_fill_keys(array_keys($monthlyData[1]), 0);

            $employeePayrolls = EmployeePayroll::where('employee_id', $employee->id)
                ->whereHas('payroll', function ($q) use ($business, $year) {
            ...
            if ($employeePayrolls->isEmpty()) {
                Log::info('No payroll data for employee — using zero defaults', ['employee_id' => $employee->id]);
            }

            foreach ($employeePayrolls as $ep) {
                $month = (int) $ep->payroll->payrun_month;

                $deductions = is_string($ep->deductions) ? json_decode($ep->deductions, true) : [];
                if (json_last_error() !== JSON_ERROR_NONE) $deductions = [];

                $basic = (float) $ep->basic_salary ?? 0;
                $gross = (float) $ep->gross_pay ?? 0;
                $taxable = (float) $ep->taxable_income ?? 0;
                $paye = is_numeric($ep->paye) ? (float) $ep->paye : (float) ($deductions['paye'] ?? 0);
                $personalRelief = is_numeric($ep->personal_relief) ? (float) $ep->personal_relief : 2400;
                $insuranceRelief = is_numeric($ep->insurance_relief) ? (float) $ep->insurance_relief : 0;

                $retirementE1 = $basic * 0.3;
                ...
                $deduction = $retirementContribution + $housingLevy + $shif + $prmf + $ownerOccupied;
                $chargeable = $gross - $deduction;
                $payeTax = $paye;
                ...
                $monthlyData[$month] = [
                    ...
                    'owner_occupied_interest' => $ownerOccupied,
                    ...
                    'tax_charged' => $payeTax + $personalRelief + $insuranceRelief,
                    'personal_relief' => $personalRelief,
                    'insurance_relief' => $insuranceRelief,
                    'paye' => $payeTax,
                ];
                ...
                foreach ($totals as $key => $val) {
                    $totals[$key] += $monthlyData[$month][$key] ?? 0;
                }
            }

            $data = [[
                'employee_name' => $business->company_name ?? $business->name,
                'main_name' => $user->name ?? '',
                'pin' => $user->kra_pin ?? 'N/A',
                'tax_no' => $business->tax_pin_no ?? 'N/A',
                'monthly_data' => $monthlyData,
                'totals' => $totals,
                'nssf' => $employee->nssf ?? 'N/A',
                'shif' => $employee->shif ?? 'N/A',
            ]];
            ...
            if (!view()->exists('payroll.reports.p9')) {
                Log::error('P9 view missing.');
                return RequestResponse::serverError('P9 view not found.');
            }

            try {
                $pdf = Pdf::loadView('payroll.reports.p9', $viewData)->setPaper('A4', 'landscape');
        ...
        Log::info('Final P9 success count', ['successCount' => $successCount]);


        if ($successCount === 0) {
            return RequestResponse::ok('P9 forms generated with zero-filled values; no real payroll data found.');
        }
    ...
    if ($ep) {
        ...
        $retirementE2 = (float) ($deductions['retirement_contribution'] ?? 0);
                    ...
                    $monthlyData[$month] = [
                        ...
                        'retirement_e1' => $retirementE1,
                        'retirement_e2' => $retirementE2,
                        'retirement_e3' => 20000, // KRA fixed limit
