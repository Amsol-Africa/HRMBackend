...
window.getFormulas = async function (page = 1) {
    try {
        ...
        Swal.fire('Error!', error.message || 'Failed to load formulas.', 'error');
    }
};
...
window.saveFormula = async function (btn) {
    ...
    try {
        ...
        Swal.fire('Error!', error.message || 'Failed to save formula.', 'error');
    } finally {
...
window.editFormula = async function (btn) {
    ...
    try {
        ...
        Swal.fire('Error!', error.message || 'Failed to load edit form.', 'error');
    }
};
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                ...
                Swal.fire('Error!', error.message || 'Failed to delete formula.', 'error');
            } finally {
                btn_loader(btn, false);
            }
        } else {
