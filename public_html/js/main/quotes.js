...
window.saveTourPackage = async function (btn) {
    ...
    // Attach itinerary data
    const itineraries = [];
    $('#itineraryContainer .itinerary-item').each(function () {
        let itinerary = {
            day: $(this).find('[name="day[]"]').val(),
            activity_title: $(this).find('[name="activity_title[]"]').val(),
            description: $(this).find('[name="description[]"]').val(),
            destination: $(this).find('[name="destination[]"]').val(),
            meals: $(this).find('[name="meals[]"]').val(),
            drinks: $(this).find('[name="drinks[]"]').val(),
            accommodation: $(this).find('[name="accommodation[]"]').val(),
            image: $(this).find('[name="image[]"]')[0].files,
            accommodation_images: $(this).find('[name="accommodation_images[]"]')[0].files,
        };
