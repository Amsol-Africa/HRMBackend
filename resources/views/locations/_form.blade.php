<form id="locationsForm">
    ...
    <div class="form-group mb-3">
        ...
        <select id="company_size" name="company_size" required class="form-select">
            ...
            <option value="1-10"
                {{ isset($location) && !empty($location) && $location->company_size === '1-10' ? 'selected' : '' }}>1-10
                employees</option>
            <option value="11-50"
                {{ isset($location) && !empty($location) && $location->company_size === '11-50' ? 'selected' : '' }}>
                11-50
                employees</option>
            <option value="51-200"
                {{ isset($location) && !empty($location) && $location->company_size === '51-200' ? 'selected' : '' }}>
                51-200 employees</option>
            <option value="201-500"
                {{ isset($location) && !empty($location) && $location->company_size === '201-500' ? 'selected' : '' }}>
                201-500 employees</option>
            <option value="500+"
                {{ isset($location) && !empty($location) && $location->company_size === '500+' ? 'selected' : '' }}>500+
                employees</option>
        </select>
    </div>
